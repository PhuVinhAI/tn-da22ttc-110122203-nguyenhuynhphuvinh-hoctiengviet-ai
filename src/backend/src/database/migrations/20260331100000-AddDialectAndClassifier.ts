...
export class AddDialectAndClassifier20260331100000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN "preferred_dialect"`,
    );

    // Drop columns from vocabularies
    await queryRunner.query(`ALTER TABLE "vocabularies" DROP COLUMN "region"`);
    await queryRunner.query(
      `ALTER TABLE "vocabularies" DROP COLUMN "audio_urls"`,
    );
    await queryRunner.query(
      `ALTER TABLE "vocabularies" DROP COLUMN "dialect_variants"`,
    );
    await queryRunner.query(
      `ALTER TABLE "vocabularies" DROP COLUMN "classifier"`,
    );
